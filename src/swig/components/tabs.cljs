...
(def tabs-args-desc
  [{:name :model     :required true                  :type "unique-id | atom"                                      :description "the unique identifier of the currently selected tab"}
   ...
   {:name :view-id   :required false :default nil    :type "view id"                  :validate-fn number?         :description "View ID"}])
...
(defn horizontal-tabs
  [& {:keys [model tabs on-change id-fn label-fn class style attr view-id]
       ...
       (let [id        (id-fn  t)
         ...
         [:button
          {:type     "button"
           ...
           :style    style
           :on-mouse-down (fn [e] (println "MOUSE DOWN!!") e)
           :on-click (when on-change (handler-fn (on-change id)))}
          label]))]))
